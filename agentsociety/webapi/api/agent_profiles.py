...
class SaveProfileRequest(BaseModel):
    ...
    description: Optional[str] = Field(None, description="Description for the saved profile")
    agent_type: str = Field(..., description="Type of agent (citizen, firm, etc.)")
...
async def get_agent_profile(
    ...
    async with request.app.state.get_db() as db:
        ...
        stmt = select(AgentProfileData).where(
            AgentProfileData.tenant_id == tenant_id,
            AgentProfileData.id == profile_id
        )
        result = await db.execute(stmt)
...
async def upload_agent_profile(
    ...
    try:
        ...
        profile_description = description if description else f"Uploaded file: {filename}"
...
async def generate_agent_profiles(
    ...
    if env.s3.enabled:
        ...
        return ApiResponseWrapper(data={
            ...
            "data": preview_data[:10]  # Return first 10 items for immediate preview
        })
    else:
        return ApiResponseWrapper(data={
            "preview_id": preview_id,
    ...
    try:
        ...
        async with request.app.state.get_db() as db:
            ...
            new_profile = AgentProfileData(
                ...
                description=profile_req.description or f"Generated profile for {profile_req.agent_type}",
