...
from ...agent import Agent, Block, FormatPrompt, param_docs
...
Current temperature: {get_temperature}
Other information: 
-------------------------
...
Current location: {get_current_position} 
Current time: {get_nowtime}
My income/consumption level: {consumption}
My occupation: {occupation}
My age: {age}
My emotion: {emotion_types}
My thought: {thought}
...
class PlanBlock(Block):
    ...
    def __init__(
        ...
        self.detail_prompt.associate_with_method(self.generate_detailed_plan)
        self.trigger_time = 0
    ...
    @param_docs(
        plan_target="The target for generating action plan",
    )
    async def generate_detailed_plan(self, plan_target: str, max_plan_steps: int) -> Optional[dict]:
